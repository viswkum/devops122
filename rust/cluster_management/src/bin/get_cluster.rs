...
/// Get the details of the provided DSQL cluster.
pub async fn get_cluster(region: &str, identifier: &str) -> anyhow::Result<GetClusterOutput> {
    let client = dsql_client(region).await?;
    let result = client.get_cluster().identifier(identifier).send().await?;
